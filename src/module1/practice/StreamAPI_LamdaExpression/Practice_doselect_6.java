...
class Implementation{
  ...
  {
    Map<String, Long> countListBygender =  list.stream().collect(Collectors.groupingBy(Candidate::getGender,HashMap::new,Collectors.counting()));
    return countListBygender;
  }

  public Map<String, Double> getAverageAge(List<Candidate> list)
  {
    Map<String, Double> avgAgeBygender =  list.stream().collect(Collectors.groupingBy(Candidate::getGender,Collectors.averagingInt(Candidate::getAge)));
  ...
  {
    Map<String, Long> countListByDept =  list.stream().collect(Collectors.groupingBy(Candidate::getDepartment,HashMap::new,Collectors.counting()));
    return countListByDept;
  }

 public Optional<Candidate>  getYoungestCandidateDetails(List<Candidate> list)
 {
	 return list.stream().filter(e->e.getGender().equalsIgnoreCase("Male") && e.getDepartment().equalsIgnoreCase("Product Development")).min(Comparator.comparing(Candidate::getAge)
			 .thenComparing(Candidate::getName,Comparator.reverseOrder()));
 }

}

public class Practice_doselect_6 {
